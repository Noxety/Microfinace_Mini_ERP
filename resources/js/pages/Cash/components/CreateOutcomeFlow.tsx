...
export default function CreateOutcomeFlow({ branches }: Props) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault()
        post(route('cash-ledgers.outcome.store'), {
            onSuccess: () => {
                toast.success('OutFlow recorded successfully')
                reset()
            },
            onError: () => {
                toast.error('Failed to record OutFlow')
            },
        })
    }
    ...
    return (
        <Card className="max-w-xl m-4">
            <CardHeader>
                <CardTitle>Create Branch OutFlow</CardTitle>
            </CardHeader>
